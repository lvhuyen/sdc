...
//import com.nbnco.csa.analysis.copper.sdc.flink.operator.SdcInstantAverager
...
//
//object StreamingSdcWithAverage {
//	object OperatorId {
...
//
//		val cfgElasticSearchSdcIndexName = appConfig.get("sink.elasticsearch.sdc.index-name", "copper-sdc-combined-default")
//		val cfgElasticSearchSdcParallelism = appConfig.getInt("sink.elasticsearch.sdc.parallelism", 4)
//
...
//				.assignTimestampsAndWatermarks(new SdcTimeAssigner[SdcRaw]).name("Assign Timestamp")
...
//				new DataStreamUtils(enrichedInstant)
...
//						.trigger(new SdcInstantAverager.AggTrigger(Time.minutes(cfgRollingAverageSlideInterval).toMilliseconds))
//						.aggregate(new SdcInstantAverager.AggIncremental, new SdcInstantAverager.AggFinal(cfgRollingAverageSlideInterval))
...
//					.setParallelism(cfgElasticSearchSdcParallelism)
//					.uid(OperatorId.SINK_ELASTIC_COMBINED)
//					.name("ElasticSearch - Combined")
//
//			if (cfgElasticSearchEnrichmentEnabled) streamEnrichment
//					.addSink(SdcElasticSearchSink.createSink[FlsRecord](
//						cfgElasticSearchEndpoint,
...
//					.name("ElasticSearch - Enrichment")
//
//			if (cfgElasticSearchUnenrichableEnabled) streamNotEnrichable
...
//					.name("ElasticSearch - Unenrichable")
//
//			if (cfgElasticSearchAverageEnabled)
//				streamAverage
//					.addSink(SdcElasticSearchSink.createSink[SdcAverage](
//						cfgElasticSearchEndpoint,
...
//					.name("ElasticSearch - Rolling Average")
//
